...
lam_fact=25;                 % A constant used for setting up grid size. 
                             % Take lam_fact >= 20 
...
time_fact=2;% A constant >1 used for setting up the time step.
           % 2 is taken for safety as there is STM modulation in particular
...
krefW=(Ny-1)*kref*dy; %Reevaluate the fimensionless transverse length 
...
Nz=floor(z_domain_size_factor*2*scat_width);   
                               % Longitudal dimensionless thickness kref*L
start_slab1=floor((Nz/2)-2*(scat_width)); 
start_slab2=floor((Nz/2)+1.3*(scat_width)); 
...
epsilon_ij(2:end-1,start_slab1:start_slab1+scat_width-1)= ...
 ...
 epsilon_ij(2:end-1,start_slab2:start_slab2+scat_width-1)= ...
    macropixels(scat_width,Ny,macropixel_size,macropixel_size,eps_avg,sigma);
              % The function macropixel returns a random grid of dielectric
              %                       constant as a function of z and y.                            
%------------------ Plot the disorder -------------------------------------
...
if ((c0/eta_min)*dt/dz) > 1/sqrt(2)
    ...
    sprintf('Good that the CFL constant = %f is less than 1/sqrt(2)=0.7071',...
        ((c0/eta_min)*dt/dz))
end
...
init_data.start_slab2=start_slab2;
