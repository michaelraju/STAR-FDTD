...
NT_steady_state_forward_propagation=28000; % No of time steps for forward 
                                         %   propagation
NT_steady_state_PC=28000; % No of time steps for phase conjugation
...
nref=1.6;                % Background(reference medium) refractive index 
sigma=0.6;               % A parameter, usually < 1 used to control the  
...
figure
r_count=1;
filename=sprintf('Efield_array_collection_PC_R2L_%d.mat',r_count);
load(filename)
field_mag_at_STM=abs(Efield_array_collection.EnTFSF(:,STM_data.STM_start_z));
plot(mag2db(field_mag_at_STM./max(field_mag_at_STM)),...
    (0:init_data.Ny+1).*init_data.kref*init_data.dy,'-*','MarkerSize',1)
...
legend(sprintf('$PC~no~%d:~Right~to~left$',r_count),...
    sprintf('$PC~no~%d:~left~to~right$',r_count),'Interpreter','Latex')
...
line([0 -40],...
    ...
    'Color','magenta','LineStyle','--','LineWidth',1)
 db_background=-6.021; %
 line([db_background db_background],[STM_y_index(1).*init_data.kref*init_data.dy STM_y_index(end).*init_data.kref*init_data.dy], ...
     'Color','magenta','LineStyle','--','LineWidth',1)
%--------------------------------------------------------------------------
subplot(1,2,2)
eta_STM_to_plot=sqrt(STM_data.eps_ij_n(1));
...
krefDfocus=(max(find(norm_dB_intensity_at_STM(STM_y_index) > db_background)) ...
    - min(find(norm_dB_intensity_at_STM(STM_y_index) > db_background))).*init_data.kref*init_data.dy;
sprintf('Width of the focal region is %f \n ',krefDfocus)
krefDSTM=(STM_y_index(end)-STM_y_index(1)).*init_data.kref*init_data.dy;
sprintf('Width of the STM region krefDfoc %f',krefDSTM)
sprintf('The ratio is %f',krefDSTM/krefDfocus)

side_description={sprintf('$k_{ref}D_{STM}=%.2f$',krefDSTM),...
                  sprintf('$k_{ref}D_{focus}=%.2f$',krefDfocus),...
                  sprintf('$(k_{ref}D_{STM})/(k_{ref}D_{focus})=%.2f$',krefDSTM/krefDfocus)};
annotation('textbox', [0.65, 0.95, 0.001, 0.001], 'string', ...
