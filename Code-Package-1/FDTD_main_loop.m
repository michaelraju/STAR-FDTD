...
%-- Setting the generalised Mur absorbing (outgoing ) Boundary Condition --
...
line([(TFSF_Nz-1)*kref*dz  (TFSF_Nz-1)*kref*dz ],[0 Ny*kref*dy], ...
    'color','w','LineWidth',2);
    % This is the TFSF boundary plotted on one side of the source.
hold on
...
% soft source allows scattered waves to pass through it without
%           undergoing additional reflection.
Esrc_n(src_y,src_z)=Esrc_n(src_y,src_z)+ ...
    source_function(t_count,wave_amp,wave_phase,t_sign); % soft dipole source
%------------- Enforce the transverse mirror boundary condition -----------
En(mirror_pos,:)=0;       % setting 0 reflects the wave
En(end-mirror_pos,:)=0;   
Esrc_n(mirror_pos,:)=0;
