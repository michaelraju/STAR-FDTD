...
%------------------- STM parameters ----------------------------------------
[STM_data]=define_STM_parameters(GPU_flag,init_data,source_data,del_eps,spat_freq_fact,f_mod,grat_width);
%------------ Forward FDTD wave propagation -------------------------------
tic
[Efield_array_collection] = FDTD_main_loop_STM(GPU_flag,visualisation_flag,init_data,source_data,STM_data);
...
PSD_array_collection=PSD_nth_SideBand(GPU_flag,visualisation_flag,Efield_array_collection,init_data,source_data,STM_data,nth);
...
for ncount=1:length(norder_array)
nth=norder_array(ncount);
plot(mag2db(PSD_structure_collection(ncount).E_trans_sig./max(peak_val_store)),krefy_detec,'-*','MarkerSize',1)
