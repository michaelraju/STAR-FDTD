...
src_z_start=gpuArray(source_data.src_z);
...
dt=init_data.dt;
fsrc_SB=fsrc+f_mod;  % modulated frequency taken to be the up-converted one
%(grat_width-1)*kref*dz

%------------ Define the geometrical parameters of the STM region ---------
STM_start_z=src_z_start(1) + floor(1*grat_width);  % z position starting
                                                    %     for STM
STM_end_z=STM_start_z+grat_width-1;  % z position ending for STM 
STM_start_y=src_y(floor(numel(src_y)/2))-floor(1.2*beam_width); 
                                  % y position starting for STM
STM_end_y=src_y(floor(numel(src_y)/2))+floor(1.2*beam_width);
                                  % y position ending for STM
[~,krefY]=meshgrid((0:Nz-1).*(kref*dz),(0:Ny-1).*(kref*dy));

omega_mod=2*pi*f_mod;     % STM temporal modulation frequency
kmod=spat_freq_fact*kref; % STM spatial modulation frequency
...
gaussian_STM=exp(-((krefY-STM_center_krefy_loc).^2)./(STM_width_fact^2));
             % For the STM spatial Gaussian modulation to be multiplied
%-------------------------- grating ---------------------------------------
% Without spatial Gaussian modulation and full size
eps_ij_n    = @(t_count) (epsilon_ij(2:end-1,2:end-1) + ...
    mask.*del_eps.*(1+sin(spat_freq_fact.*krefY-omega_mod*(t_count-1)*dt)));
...
% Here, the temporal derivatives are analytically estimated 
%----------------------------- STM_data -----------------------------------
...
colormap jet
imagesc([0 kref*dz*(Nz-1)],[0 kref*dy*(Ny-1)],sqrt(eps_ij_n(1)));  
